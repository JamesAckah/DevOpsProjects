...
6.	![image](https://github.com/user-attachments/assets/1e42bbfe-741a-4d8b-b56c-e872e24f7fa5)

7.	In the use case select Lambda and click on next.

8.	![image](https://github.com/user-attachments/assets/10053c4b-c2e2-42d3-aba7-e67bf08f3568)
   
10.	In the add permission page select AWSLambdaRole policy by typing it in the search bar and click next.
...
9.	You have successfully created the Lambda Role. Make sure to copy the ARN of the role created and save it in a notepad or any text editor.
...
1.	In the AWS Management Console, locate the CloudShell icon in the top navigation bar. It looks like a terminal/console icon.
...
3.	A new tab in your browser opens, and you will see a welcome message to the cloud shell, click on the Close button in that message.

![image](https://github.com/user-attachments/assets/bb389843-8bc0-4061-a830-2c6fa8caaa68)

4.	You will see a creating environment message on the screen. Wait for a few minutes to complete the environment creation. Once the environment is created. You are ready to use the terminal.
**NB:** With AWS CloudShell, you can use up to 1 GB of persistent storage in each AWS Region at no additional cost. Persistent storage is located in your home directory ($HOME) and is private to you. Unlike ephemeral environment resources that are recycled after each shell session ends, data in your home directory persists between sessions.
...
1.	We’ll now write a Python script that interacts with an S3 bucket. The function will create a bucket and upload a file. Below is the Python script. 
...
3.	Copy the python script above and paste it into the editor and save it using Ctrl + O and hint enter, and Ctrl + X to exit from the editor. You can use your preferred editor for writing the python script.

4.	![image](https://github.com/user-attachments/assets/89648f94-7f90-4ce3-92f8-d822fe83064d)

5.	4.	Create a Zip file of the s3bucket.py file, which is used to create lambda function in CLI using below command. 
zip s3bucket.zip s3bucket.py
5.	Create a lambda function from CLI using the following command. 
aws lambda create-function --function-name lambdaclidemo --runtime python3.12 --zip-file fileb://s3bucket.zip --handler s3bucket.handler --role arn:aws:iam::533267168161:role/LambdaRoleVersion
...
7.	![image](https://github.com/user-attachments/assets/b5b8dd6b-7a9b-4bfc-bca9-00f887e3223c)

8.	**TASK 5: UPDATING AND INVOKING THE LAMBDA FUNCTION:**
9.	
...
3.	![image](https://github.com/user-attachments/assets/c720fbbc-715d-46fd-b270-2803bc880a45)

4.	Let us change the content of the file and the name of the file and upload it to s3.
   
3.	First, navigate to CloudShell and open the file using the nano editor using the below command. 
     Nano s3bucket.py
...
5.	Now remove the existing zip file s3bucket.zip and create a new zip file with updated codes using the below commands. 
    rm -f s3bucket.zip         
    zip s3bucket.zip s3bucket.py
...
1.	To create an Alias for the Lambda function run the below command, here we are creating Alias for the lambdaclidemo 
   ...
   2.	To check the Alias created, Navigate to the Lambda dashboard and choose a function -->Alias and you can see the alias created for version 1.
   3.	![image](https://github.com/user-attachments/assets/db84dfee-065e-4f22-988a-76632759e849)

   4.	3.	You can create another alias for the PROD team using the command above by changing the version to 2 and the name 
          to PROD. 
